...
public class DDLStmtExecutor {
    ...
    public static class StmtExecutorVisitor implements AstVisitor<ShowResultSet, ConnectContext> {
        ...
        public ShowResultSet visitCancelAlterTableStatement(CancelAlterTableStmt stmt, ConnectContext context) {
            ErrorReport.wrapWithRuntimeException(() -> {
                context.getGlobalStateMgr().getLocalMetastore().cancelAlter(stmt, context);
            });
            return null;
        }
        ...
        public ShowResultSet visitDropTaskStmt(DropTaskStmt dropTaskStmt, ConnectContext context) {
            ...
            Authorizer.checkDropTask(context, task, dropTaskStmt.isForce());
            if (!dropTaskStmt.isForce()
                    && (task.getSource() == Constants.TaskSource.MV
                    || task.getSource() == Constants.TaskSource.PIPE)) {
                throw new RuntimeException("Can not drop task generated by materialized view or pipe. Use the " +
                        "corresponding DROP MATERIALIZED VIEW or DROP PIPE statement instead.");
            }
        ...
        public ShowResultSet visitCreateDictionaryStatement(CreateDictionaryStmt stmt, ConnectContext context) {
            Authorizer.checkDictionaryCreateBeforeAnalyze(stmt, context);
            ErrorReport.wrapWithRuntimeException(() -> {
                context.getGlobalStateMgr().getDictionaryMgr().createDictionary(stmt,
                        stmt.getQueryableObjectName().getCatalog(), stmt.getQueryableObjectName().getDb());
            });
            return null;
        }
